...
async function removeTag(petaTag: RPetaTag) {
  if (
    (await IPC.common.openModal(t("browser.removeTagDialog", [petaTag.name]), [
      t("commons.yes"),
      t("commons.no"),
    ])) === 0
  ) {
...
async function removeTagPartition(petaTag: PetaTagPartition) {
  if (
    (await IPC.common.openModal(t("browser.removeTagPartitionDialog", [petaTag.name]), [
      t("commons.yes"),
      t("commons.no"),
    ])) === 0
  ) {
...
async function changeTag(petaTag: RPetaTag, newName: string) {
  ...
  if (browserTags.value.find((c) => c.petaTag.name === newName)) {
    await IPC.common.openModal(t("browser.tagAlreadyExistsDialog", [newName]), [t("commons.yes")]);
    return;
  }
